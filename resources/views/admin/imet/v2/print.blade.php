<?php
/** @var \App\Models\Imet\v2\Imet $item */

// Force Language
if($item->language != App::getLocale()){
    App::setLocale($item->language);
}

?>

@extends('layouts.components._base')

@section('body')

    <style>
        .print_body{
            margin: 20px;
        }
        .entity-heading{
            margin-top: 20px;
        }

    </style>

    <div id="print_body" class="print_body">

        <a href="{{ action([\App\Http\Controllers\Imet\ImetController::class, 'index']) }}" class="btn-nav rounded" style="margin-bottom: 20px;">{{ ucfirst(trans('common.go_back')) }}</a>

        <section class="container-fluid header-ribbon">
            <span class="page_title">
               @lang('form/imet/common.imet')
            </span>
        </section>

        <div class="entity-heading">
            <div class="id">#{{ $item->getKey() }}</div>
            <div class="name">{{ $item->Name }}</div>
            <div class="location">{!! \App\Library\Ofac\Template::flag_and_name($item->Country) !!}</div>
        </div>


        {{-- management effectiveness --}}
        @include('admin.imet.v2.evaluation.management_effectiveness.management_effectiveness', [
            'item_id' => $item->getKey(),
            'step' => 'management_effectiveness'
        ])

        {{--  Modules (context) --}}
        <h1>{{ ucfirst(trans('form/imet/common.context_long')) }}</h1>
        <div class="imet_modules">
            @foreach($item::modules() as $step=>$modules_step)
                @foreach($modules_step as $i=>$module)
                    @include('admin.components.module.preview.container', [
                        'module_class' => $module,
                        'form_id' => $item->getKey()])
                @endforeach
            @endforeach
        </div>

        {{--  Modules (evaluation) --}}
        <h1>{{ ucfirst(trans('form/imet/common.evaluation_long')) }}</h1>
        <div class="imet_modules">
            @foreach(\App\Models\Imet\v2\Imet_Eval::modules() as $step=>$modules_step)
                @foreach($modules_step as $i=>$module)
                    @include('admin.components.module.preview.container', [
                        'module_class' => $module,
                        'form_id' => $item->getKey()])
                @endforeach
            @endforeach
        </div>

    </div>

    <script>
        window.onload = function () {
            setTimeout(function(){
                window.print();
            }, 2000);
        }
    </script>

@endsection