...
?>
    ...
    @elseif($definitions['module_type']==='TABLE' || $definitions['module_type']==='ACCORDION')
        ...
        @foreach($records as $year=>$record)
            @if(!empty($record))
                <button type="button" class="btn btn-sm act-btn-basic preload_button year_{{ $year }}" onclick="module_{{ $definitions['module_key'] }}.show_previous_year_concession('{{ $year }}')">{{ $year }}</button>&nbsp;
            @else
                        ...
                        <tbody>
                            @foreach($record_year as $index=>$record)
                                <tr>
                                    ...
                                    @endforeach
                                    <td>
                                        <button type="button" class="btn btn-sm btn-success" onclick="module_{{ $definitions['module_key'] }}.apply_preload_one_record_concession('{{ $year }}', '{{ $index }}')">{{ ucfirst(trans('common.import')) }}</button>
                                    </td>
                                </tr>
                            @endforeach
                            </tr>
