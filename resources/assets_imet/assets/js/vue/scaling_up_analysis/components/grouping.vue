  ...
  <div>
    <div class='row' id="js-grouping-action-buttons">
      <div class="col-24 mb-2">
        <button type="button" class="btn btn-success mb-1" @click="add_by_country">{{ stores.BaseStore.localization('form/imet/analysis_report/report.grouping.add_country') }}</button>
        <button type="button" class="btn btn-danger mb-1" @click="reset">{{ stores.BaseStore.localization('form/imet/analysis_report/report.grouping.reset') }}</button>
        <button type="button" @click="add_group()" class="btn btn-primary mb-1"><i class="fa fa-plus"
                                                                                   aria-hidden="true">{{ stores.BaseStore.localization('form/imet/analysis_report/report.grouping.add_group') }}</i></button>
      </div>
    </div>
    ...
    <div class="row" id="js-render-buttons">
      <div class="col-24 mt-5 mb-5">
        <button type="button" @click="show_diagrams" class="btn btn-success">
          {{ stores.BaseStore.localization('form/imet/analysis_report/report.grouping.render_radar') }}
        </button>
        <button type="button" @click="show_diagrams({func:'get_scatter_grouping_analysis'})" class="btn btn-success">
          {{ stores.BaseStore.localization('form/imet/analysis_report/report.grouping.render_scatter') }}
        </button>
      </div>
    </div>
    <div id="render_image">
...
export default {
  ...
  methods: {
    ...
    show_diagrams: function (params = null) {
      ...
      if (render) {
        this.$root.$emit('incoming-data', {
          ...
          parameters: this.list.filter(item => item.list !== null).map(item => {
            let group_name = `${this.stores.BaseStore.localization('form/imet/analysis_report/report.grouping.group')} ${item.list}`;
            if (item.by_country) {
