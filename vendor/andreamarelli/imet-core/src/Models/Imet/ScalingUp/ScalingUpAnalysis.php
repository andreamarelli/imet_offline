...
class ScalingUpAnalysis extends Model
{
     ...
     * @return array
     */
    public static function get_maps_stats(array $form_ids): array
    {
        $country_indicators = [];
        $dopa_stats = ['all_data' => [],
    ...
    /**
     * get the theats categories for the protected areas by form ids
     * @param array $form_ids
     * @return array
     */
    ...
    private static function analysis_diagram_protected_areas(array $form_ids, string $type, array $table_indicators, array $options, string $custom_type): array
    {
        ...
        foreach ($radar_average as $k => $item) {
            $radar_protected_areas['values']['Average'][] = Common::round_number($item / count($form_ids));
        }
