<?php

return [

    'management'        => 'IMET management',
    'v1' => 'v1',

    'indicator'     => 'Indicator',
    'methodology'   => 'Methodology',
    'criteria'      => 'Criteria - Measured concept  - Variables',


    'steps' => [
        'general_info'          => 'General Information',
        'areas'                 => 'Areas, boundaries and shape index, level of control',
        'resources'             => 'Human, financial and material resources',
        'key_elements'          => 'Key elements',
        'threats'               => 'Threats',
        'climate'               => 'Climate change and conservation',
        'ecosystem_services'    => 'Ecosystem Services and community dependence',
        'objectives'            => 'Objectives and Management',
    ],

    'steps_eval' => [
        'general_info'              => 'General Information',
        'context'                   => 'Management context',
        'planning'                  => 'Planning',
        'inputs'                    => 'Inputs',
        'process'                   => 'Process',
        'outputs'                   => 'Outputs',
        'outcomes'                  => 'Outcomes',
        'management_effectiveness'  => 'Management Effectiveness',
    ],

    'indexes' => [
        'imet'        => 'IMET index',
        'context'     => 'Context index',
        'planning'    => 'Planning index',
        'inputs'      => 'Inputs index',
        'process'     => 'Process index',
        'outputs'     => 'Outputs index',
        'outcomes'    => 'Outcomes index',
    ],

    'assessment' => [
        'ctx101' => [
            'CTX 1.0.1',
            'Responsibility for filling the form: Management team and partners and external support for analysis and management evaluation'
        ],
        'ctx102' => [
            'CTX 1.0.2',
            'Responsibility for completing the form: Management team and partners and external support for analysis and management evaluation'
        ],
        'c11' => [
            'C1.1',
            'Governance/Partnership'
        ],
        'c12' => [
            'C1.2',
            'Classifications'
        ],
        'c13' => [
            'C1.3',
            'Species'
        ],
        'c14' => [
            'C1.4',
            'Terrestrial and marine habitats and land-cover and land-change'
        ],
        'c15' => [
            'C1.5',
            'Climate Change'
        ],
        'c16' => [
            'C1.6',
            'Ecosystem services'
        ],
        'c1' => [
            'C1',
            'Value and Importance'
        ],
        'c2' => [
            'C2',
            'External constraints or supporting factors'
        ],
        'c3' => [
            'C3',
            'Threats'
        ],
        'p1' => [
            'P1',
            'Adequacy of legal and regulatory provisions'
        ],
        'p2' => [
            'P2',
            'Design and layout of the protected area'
        ],
        'p3' => [
            'P3',
            'Demarcation of the protected area'
        ],
        'p4' => [
            'P4',
            'Management plan'
        ],
        'p5' => [
            'P5',
            'Work plan'
        ],
        'p6' => [
            'P6',
            'Objectives of the protected area'
        ],
        'i1' => [
            'I1',
            'Basic information'
        ],
        'i2' => [
            'I2',
            'Staff'
        ],
        'i3' => [
            'I3',
            'Current budget'
        ],
        'i4' => [
            'I4',
            'Securing the budget'
        ],
        'i5' => [
            'I5',
            'Infrastructure, equipment and facilities'
        ],
        'pr1' => [
            'PR1',
            'Staff capabilities and training'
        ],
        'pr2' => [
            'PR2',
            'Human resource management policies and procedures'
        ],
        'pr3' => [
            'PR3',
            'Analyse the degree of staff motivation (job suitability)'
        ],
        'pr4' => [
            'PR4',
            'Management and internal leadership'
        ],
        'pr5' => [
            'PR5',
            'Accounting and financial management'
        ],
        'pr6' => [
            'PR6',
            'Maintenance of infrastructure, equipment and facilities'
        ],
        'pr7' => [
            'PR7',
            'Managing the values and important aspects of the protected area'
        ],
        'pr8' => [
            'PR8',
            'Degree of protection of the values and important aspects of the protected area'
        ],
        'pr9' => [
            'PR9',
            'Control of the protected area'
        ],
        'pr10' => [
            'PR10',
            'Enforcing the law'
        ],
        'pr11' => [
            'PR11',
            'Involvement of communities, beneficiaries and stakeholders'
        ],
        'pr12' => [
            'PR12',
            'Appropriate benefits/assistance for local communities'
        ],
        'pr13' => [
            'PR13',
            'Relations with the stakeholders and Environmental education'
        ],
        'pr14' => [
            'PR14',
            'Visitor management'
        ],
        'pr15' => [
            'PR15',
            'Visitors and impacts'
        ],
        'pr16' => [
            'PR16',
            'Monitoring systems for natural and cultural resources'
        ],
        'pr17' => [
            'PR17',
            'Research and biomonitoring'
        ],
        'pr18' => [
            'PR18',
            'Management of the effects of climate change'
        ],
        'pr19' => [
            'PR19',
            'Ecosystem services'
        ],
        'r1' => [
            'R1',
            'Implementation of the programme of work'
        ],
        'r2' => [
            'R2',
            'Outputs achieved'
        ],
        'ei1' => [
            'E/I1',
            'Achievement of conservation objectives'
        ],
        'ei2' => [
            'E/I2',
            'Conservation status of the designated values or the key elements of the protected area'
        ],
        'ei3' => [
            'E/I3',
            'Trends in the conservation status of the designated values or key elements of the protected area'
        ],
        'ei4' => [
            'E/I4',
            'Outcomes for local communities'
        ],
        'ei5' => [
            'E/I5',
            'Outcomes of mitigation of and adaptation to climate change'
        ],
        'ei6' => [
            'E/I6',
            'Outcomes for ecosystem services'
        ],
        'pr1_6' => [
            'PR1-6',
            'Internal management'
        ],
        'pr7_10' => [
            'PR7-10',
            'Protection/Management'
        ],
        'pr11_13' => [
            'PR11-13',
            'Relations'
        ],
        'pr14_15' => [
            'PR14-15',
            'Tourism'
        ],
        'pr16_17' => [
            'PR16-17',
            'Monitoring and Research'
        ],
        'pr18_19' => [
            'PR18-19',
            'Climate change and ecosystem services'
        ],
    ]

];
