...
@section('body')
    ...
    <div id="print_body" class="print_body">
        ...
        <a href="{{ route('imet-core::index') }}" class="btn-nav rounded"
           style="margin-bottom: 20px;">@uclang('modular-forms::common.go_back')</a>
        ...
        <div class="imet_modules">
            @foreach(Imet_Eval::modules() as $step=>$modules_step)
                @foreach($modules_step as $i=>$module)
                    @if(Role::hasRequiredAccessLevel($module))
                        @include('modular-forms::module.show.container', [
                            'module_class' => $module,
