export default {
    ...
    mounted (){
        let _this = this;
        $(document).on('show.bs.modal','#preload_modal__concession'+this.module_key, function () {
            _this.__retrieve_preload_data_concession();
        });
    },

    methods: {

        __retrieve_preload_data_concession: function(){
            let _this = this;
            $.ajax({
                url: window.Laravel.baseUrl + 'api/concession/indicators/preload',
                type: "POST",
                data: {
        ...
        show_previous_year_concession: function(year){
        ...
        apply_preload_one_record_concession: function (year, index) {
            let _this = this;
